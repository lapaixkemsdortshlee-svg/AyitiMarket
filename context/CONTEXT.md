...
- **Modèle économique :** Frais d'escrow sur les transactions, sécurisées par un système d'escrow (l'argent est bloqué jusqu'à la livraison). **Qui paie, tranché le 2026-07-26 : c'est le VENDEUR.** L'acheteur paie le prix affiché, sans frais ajoutés. Taux actuel 3%, présenté non pas comme le prix d'Ekomat mais comme un **tarif fondateur bloqué à vie pour les 50 premiers vendeurs** (« menm si tarif estanda a monte pita, ou pa janm depase 3% »), affiché en tête de la vérification vendeur avec un compteur de places restantes branché sur les vendeurs actifs réels. Raison : augmenter un take rate sur une marketplace se paie très cher en confiance, un plafond à vie évite d'avoir jamais à le faire sur les premiers vendeurs. Le tarif standard sera fixé après le pilote, quand le coût réel par commande sera mesuré.
...
- **Rebrand ✅ nom choisi : EKOMAT (2026-07-11).** Thrasher a choisi le nom et créé l'identité visuelle (logo : « e »-boule rust avec anse teal + wordmark « eko » rust / « mat » teal, palette BRAND.md). L'app, les contenus, les docs et les logos (déposés par Thrasher, intégrés partout : login/splash, icônes, favicon, og-image) sont rebrandés ; projet Vercel et repo GitHub renommés. **Domaine choisi le 2026-07-25 : `ekomathaiti.com`** (11,25 $/an) : `ekomat.com` est pris par une société d'un autre pays et d'un autre secteur, donc aucun conflit de marque (droit territorial et sectoriel), et `ekomathaiti.com` ancre en plus la marque au pays. Registrar conseillé : Cloudflare (prix coûtant à vie) ou Vercel (branchement en 2 clics). **Reste** : acheter le domaine, le brancher sur Vercel et mettre à jour les URLs (`ayiti-market.vercel.app` reste l'URL active en attendant) ; redéployer les edge functions ; renommer Firebase/Google OAuth.
- **Exécuter le pilote fermé (COURT TERME, PRIORITAIRE, l'audit est fait, place au terrain).** Protocole complet : `docs/PROTOCOLE-PILOTE-FERME.md` (2026-07-17). **Décision révisée (2026-07-17, session 3) : le SMS OTP est REPORTÉ** (abonnement Twilio pas maintenant), le pilote tourne sur la vérification **CIN/Paspò seule** (validation admin manuelle, déjà fonctionnelle) et le vendeur saisit son numéro MonCash payout à la main. Il ne reste donc qu'**UNE action : recruter des vendeurs**. **Objectif revu par Thrasher le 2026-07-26 : 50 vendeurs** (il remplace l'ancien « 2-3 vendeurs contrôlés »), ce qui aligne le pilote sur le plafond du tarif fondateur. Critère de succès conservé : ≥3 transactions `released` sur ≥2 vendeurs → enchaîner sur le closed testing Google Play. Cadrage Thrasher inchangé : « pas de démo, préparer l'infra pour de vrais résultats ». Le goulot n'est plus technique du tout.
  - **Réserve Alita (sparring, exprimée et écartée par Thrasher le 2026-07-26) :** à 50 vendeurs ce n'est plus un pilote mais un lancement. Le rail argent est encore manuel (Thrasher vérifie chaque référence MonCash et fait chaque payout à la main), donc un défaut sur l'escrow toucherait 50 vendeurs simultanément au lieu de 2 réparables à la main. Si l'objectif reste 50, deux garde-fous valent d'être posés : recruter par vagues (5, puis 15, puis le reste) et ne passer à la vague suivante qu'après des transactions `released` propres sur la précédente.
...
- **Comptes de la plateforme (bascule 2026-07-25) :** l'admin est désormais **`ekomatmarketplacehaiti@gmail.com`** (aussi l'email de support client affiché dans les conditions et la politique de confidentialité). L'ancien `ayitimarket01@gmail.com` est devenu **« Ekomat Boutique »**, la boutique première-partie de Thrasher : vendeur vérifié + officiel (aucune vérification CIN à faire), marqué d'un **badge or** distinct du teal des vendeurs vérifiés. Il n'a **jamais été supprimé** car il porte la piste d'audit argent. Règle retenue : l'or signifie « boutique officielle » (un état), pas « vendeur premium » (un rang), sinon les vrais vendeurs réclameraient un badge inatteignable.
- **Base remise à ZÉRO pour le pilote (2026-07-25) :** les 7 comptes existants étant tous des tests, toutes les données transactionnelles ont été supprimées (commandes, avis, notifications, vues, messages, actions admin, promos, favoris, flash deals) et les compteurs remis à 0. **Les 5 produits et les 7 profils sont conservés.** Escrow et statistiques affichent 0 : c'est voulu, pas un bug. Les comptes internes sont marqués `internal_account = true` et exclus du funnel comme des graphiques admin, donc **le premier vrai vendeur pilote produira des chiffres vrais dès le jour 1**.
- **Mémoire longue durée : ce qui survit vraiment (constaté 2026-07-26).** Le dépôt est la seule mémoire fiable entre les sessions. Agentmemory tourne dans le conteneur éphémère : ses données vivent dans `/root/.agentmemory` et disparaissent avec lui, ce qui a été mesuré directement (0 session, 0 observation, 0 mémoire, alors que HISTORY.md affirmait que des leçons y avaient été semées les 25 et 26 juillet). De plus, rien ne démarrait le serveur REST sur `:3111`, donc `memory_save` et `memory_recall` étaient purement absents des sessions ; le hook `session-start.sh` le lance désormais, ce qui règle la disponibilité **dans** une session. Pour une vraie persistance **entre** sessions, il faudrait héberger une instance agentmemory et poser `AGENTMEMORY_URL` plus `AGENTMEMORY_SECRET` dans les variables d'environnement Claude Code web (plus `AGENTMEMORY_FORCE_PROXY=1`, prévu pour les clients MCP en sandbox qui ne joignent pas localhost). **Décision Thrasher du 2026-07-26 : reporté, pas nécessaire maintenant.** Motif retenu : les leçons durables vivent déjà dans `LEARNINGS.md` et `HISTORY.md`, versionnés donc increvables, et `CLAUDE.md` impose déjà de lire `LEARNINGS.md` avant chaque action. Ce qu'un hébergement ajouterait, c'est la recherche sémantique sur des centaines de sessions, alors qu'il y en a quelques dizaines sur un seul projet. En face, un serveur payant toujours allumé et un endpoint public contenant les internes du projet. **À rouvrir quand** il y aura plusieurs projets actifs, ou quand `HISTORY.md` sera trop long pour être lu en entier. **Dette connue à traiter ce jour-là :** le hook `session-start.sh` démarre un worker local inconditionnellement ; il devra sauter ce démarrage quand `AGENTMEMORY_URL` pointe ailleurs que sur localhost, sinon on écrit dans la mémoire locale éphémère en croyant écrire dans la distante. Conséquence pratique en attendant : toute leçon qui doit survivre va dans `LEARNINGS.md` ou `context/HISTORY.md`, jamais uniquement dans agentmemory.
- **Mémoire des leçons (depuis 2026-07-25) :** skill `ecriture-pro-et-lecons` dans `~/.claude/skills`, et `LEARNINGS.md` versionné à la racine du dépôt. Le fichier enregistre les corrections de Thrasher pour qu'elles survivent aux sessions ; le skill vit hors du dépôt, d'où l'intérêt d'y garder le fichier. Les règles permanentes sont lues avant chaque action, dont l'interdiction du tiret cadratin sur tous les supports. Le bloc « Avant toute action » de CLAUDE.md rend cette relecture systématique et non occasionnelle.
